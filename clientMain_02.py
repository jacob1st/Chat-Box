...
# from tkinter.constants import BOTTOM, COMMAND, LEFT
# from typing import Collection, Text, final
...
# ###IDEAS
# # scrollwheel, disconnect needds to close, admin perms, customization
# # wrap length for label
...
# # Black frame (for entry)
# full_frame = tk.Frame(root, bg='#506D05')
# full_frame.place(relx=0, rely=0, relwidth=1, relheight=1)

# # Blue frame
# frame = tk.Frame(root, bg='#1F208D')
# frame.place(relx=0, rely=0, relwidth=1, relheight=0.9)

# #label for messages in Blue Frame
# texts_label = tk.Label(frame, bg="#1F208D", fg="white", wraplength=600, relief="ridge", font="Calibri 15", anchor="nw", justify="left", border=4)
# texts_label.place(relx=0.1, rely=0.1, relwidth=0.8, relheight=0.8)

# # Send button
# button = tk.Button(full_frame, text="send", font="Arial 12", bg="#B673E8", command=lambda: sending_messages(entry.get()))
# button.place(relx=.9, rely=0.9, relwidth=.1, relheight=.1)

# # Message label
# label = tk.Label(full_frame, text="Message: ", relief="raised", bg="#B673E8", font="Arial 12")
# label.place(relx=0, rely=0.9, relwidth=.1, relheight=.1)
